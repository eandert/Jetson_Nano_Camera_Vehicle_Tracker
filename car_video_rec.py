    ...
    for detection in detections:
        ...
        for name_key, color_val in color_dict.items():
            if name_key == name_tag:
                ...
                cv2.putText(img,
                            detection[0].decode() +
                            " [" + str(round(detection[1] * 100, 2)) + "] - d" + str(distancei) + "'",
                            (pt1[0], pt1[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            color, 2)
                            
    print ( "Time:", time.time(), " Total:" , total, " Cars:" , cars,  " Trucks:" , trucks, " Buses:" , buses)
...
class YOLO:
    ...
    def readFrame(self, frame_read):
        ...
        darknet.copy_image_from_bytes(self.darknet_image,frame_resized.tobytes())                # Copy that frame bytes to darknet_image

        detections = darknet.detect_image(netMain, metaMain, self.darknet_image, thresh=0.25)    # Detection occurs at this line and return detections, for customize we can change the threshold.                                                                                   
